...
var game = {
  ...
  function getData(data, which_game, player) {
    ...
    //only able to click in one spot
  ...
  //check every move and declares the winner
  function checkMoves(arr, player) {
    ...
    for (var i = 0; i < game.winning_array.length; i++) {
      if (
        ...
        output.innerHTML = `${player} wins! <br/> Refresh for a rematch! <br/> <button onclick=window.location.reload();>Refresh</button>`;
      }
    }
    if (game.cross.length + game.round.length == 9) {
      output.innerHTML = `Tie Game`;
    }
  }
